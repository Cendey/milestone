...
public abstract class Tree<A extends Comparable<A>> {
    ...
    private static class T<A extends Comparable<A>> extends Tree<A> {
        ...
        private Tree<A> balance(Color color, Tree<A> left, A value, Tree<A> right) {
            ...
            if (color.isB() && left.isTR() && left.right().isTR()) {
                return new T<>(
                    R, new T<>(B, left.left(), left.value(), left.right().left()), left.right().value(),
                    new T<>(B, left.right().right(), value, right));
            }
            // balance B a x (T R (T R b y c) z d) = T R (T B a x b) y (T B c z d)
            if (color.isB() && right.isTR() && right.left().isTR()) {
                return new T<>(
                    R, new T<>(B, left, value, right.left().left()), right.left().value(),
            ...
            if (color.isBB() && left.isTR() && left.right().isTR()) {
                return new T<>(
                    B, new T<>(B, left.left(), left.value(), left.right().left()), left.right().value(),
                    new T<>(B, left.right().right(), value, right));
            }
            // balance BB a x (T R (T R b y c) z d) = T B (T B a x b) y (T B c z d)
            if (color.isBB() && right.isTR() && right.left().isTR()) {
                return new T<>(
                    B, new T<>(B, left, value, right.left().left()), right.left().value(),
                    new T<>(B, right.left().right(), right.value(), right.right()));
            }
            ...
            if (color.isBB() && left.isTNB() && left.left().isTB() && left.right().isTB()) {
                return new T<>(
                    B, balance(B, redden(left.left()), left.value(), left.right().left()), left.right().value(),
                    new T<>(B, left.right().right(), value, right));
            }
        ...
        @Override
        public <B> B foldInReverseOrder(B identity, Function<B, Function<A, Function<B, B>>> f) {
            return f.apply(right.foldInReverseOrder(identity, f)).apply(value)
    ...
    private static <A extends Comparable<A>> String[][] makeTable(
        String[][] table, Tree<A> t, int hPosition, int vPosition) {
